...
const PixelPet = ({ stage, color, mode, isActive, isPetting, onClick, emotion, message }) => {
  const designs = {
    egg: [
      "0000000000000000", "0000002222000000", "0000221111220000", "0002111111112000",
      "0021113311111200", "0021133311111200", "0211111111111120", "0211111441111120",
      "0211111441111120", "0211111111111120", "0211111111111120", "0021111111111200",
      "0002111111112000", "0000221111220000", "0000002222000000", "0000000000000000"
    ],
    baby: [
      "0000000000000000", "0000000000000000", "0000000000000000", "0000000000000000",
      "0000000222000000", "0000002111200000", "0000002111112000", "0000021111111200",
      "0002111111111200", "0002122111221200", "0002123111231200", "0021111111111120",
      "0021111444111120", "0002211111112200", "0000022222220000", "0000000000000000"
    ],
    adult: [
      "0000000000000000", "0000000000000000", "0002200000002200", "0021200000021200",
      "0021122222211200", "0021111111111200", "0021122111221200", "0021123111231200",
      "0021111111111200", "0002111111112000", "0002111111112002", "0021111111111221",
      "0021111111111121", "0021111111112212", "0002222222221120", "0000000000002200"
    ],
    master: [
      "0000000000000000", "0000002222200000", "0000021111120000", "0000211111112000",
      "0000212212212000", "0002111111111200", "0002444444444200", "0024444444444420",
      "0021111111111220", "0211111111111120", "0212211111112120", "0221122222221120",
      "0002200000002200", "0000000000000000", "0000000000000000", "0000000000000000"
    ]
  };
...
const App = () => {
  ...
  const [quests, setQuests] = useState([
    ...
    { id: 3, text: '使用背景白噪音', target: 1, current: 0, reward: 20, claimed: false },
  ]);
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col font-sans transition-colors duration-700 ${themeStyles[bgTheme]}`}>
      ...
      `}>
        ...
        {/* 電腦版頂部裝飾 */}
        <div className="hidden md:flex absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-black rounded-b-xl z-20 justify-center items-center">
        ...
        <div className="px-6 pt-8 pb-4 flex justify-between items-start z-10 shrink-0">
          ...
          <div className="flex gap-2">
             <button onClick={() => setShowSounds(!showSounds)} className={`w-8 h-8 rounded-full flex items-center justify-center border shadow-sm transition-all ${isPlayingSound ? 'bg-indigo-500 text-white border-indigo-600 animate-pulse' : 'bg-white/20 border-black/5 text-slate-700'}`}>
        ...
        {showSounds && (
          <div className="mx-6 mb-2 p-3 bg-white/80 backdrop-blur-md rounded-2xl flex justify-around shadow-sm animate-in slide-in-from-top-2 border border-white/50">
             {[ {id:'rain', label:'雨聲', icon:'🌧️'}, {id:'cafe', label:'咖啡廳', icon:'☕'}, {id:'lofi', label:'Lo-Fi', icon:'🎧'} ].map(s => (
               <button 
                 key={s.id} 
        ...
        <div className="flex-grow flex flex-col items-center justify-center relative w-full px-6 py-4">
           ...
           <div className="my-8 text-center w-full">
             ...
             {/* Task Input (With Mandatory Check & Shake Animation) */}
             {isIdle && (
             ...
             {isIdle ? (
               <div className="flex flex-col gap-6 animate-in fade-in zoom-in duration-300 w-full max-w-[280px] mx-auto">
                 ...
                 <button onClick={() => setStrictMode(!strictMode)} className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-xs font-bold transition-all border shadow-sm ${strictMode ? 'bg-rose-50 border-rose-200 text-rose-600' : 'bg-white/50 border-slate-200 text-slate-400'}`}>
        ...
        <div className="pb-12 pt-4 px-8 w-full shrink-0">
           <div className="flex justify-between items-center max-w-sm mx-auto gap-4">
              <button onClick={() => setShowQuests(true)} className="w-16 h-16 rounded-full bg-emerald-100/90 backdrop-blur-sm text-emerald-700 flex items-center justify-center transition-all active:scale-90 shadow-lg relative border-2 border-white/50">
                <ScrollText size={24} />
              ...
              <button onClick={() => setShowShop(true)} className="w-16 h-16 rounded-full bg-yellow-400/90 backdrop-blur-sm text-yellow-900 flex items-center justify-center shadow-lg shadow-yellow-100 transition-all active:scale-90 border-2 border-white/50">
                <ShoppingBag size={24} />
              </button>
           </div>
        </div>
           ...
           <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end">
             <div className="bg-white w-full h-[85vh] rounded-t-[2.5rem] p-6 flex flex-col animate-in slide-in-from-bottom duration-300 shadow-2xl safe-area-bottom">
                ...
                <div className="flex justify-between items-center mb-6 px-2">
                  ...
                  <button onClick={() => setShowQuests(false)} className="p-2 bg-slate-100 rounded-full active:bg-slate-200"><X size={20} /></button>
                </div>
